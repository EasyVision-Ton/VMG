...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, q, kv=None, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(N1, N2, -1)  # D1*Wh*Ww, D2*Wh*Ww, nH
...
class VSTSREncoderTransformerBlock(nn.Module):
    ...
    """
    def __init__(self, dim, num_heads, num_frames=7, window_size=(8, 8), 
...
class UpdownkeepSampling(nn.Module):
    def __init__(self, dim_in, dim_out, mode='keep'):
        super(UpdownkeepSampling, self).__init__()
        self.dim_in = dim_in
        self.dim_out = dim_out
        self.mode = mode
        if mode == 'down':
            self.norm = nn.LayerNorm(self.dim_in*2*2)
            self.linear = nn.Linear(self.dim_in*2*2, self.dim_out)
        elif mode == 'up':
            self.norm = nn.LayerNorm(self.dim_in//2//2)
            self.linear = nn.Linear(self.dim_in//2//2, self.dim_out)
        elif mode == 'keep':
    ...
    def forward(self, x):
        assert x.ndim == 5
        if self.mode == 'keep':
            x = rearrange(x, 'n d c h w -> n d h w c').contiguous()
            x = self.norm(x)
            x = self.linear(x)
            x = rearrange(x, 'n d h w c -> n d c h w').contiguous()
        elif self.mode == 'down':
            x = rearrange(x, 'n d c (h neih) (w neiw) -> n d h w (neiw neih c)', neih=2, neiw=2).contiguous()
            x = self.norm(x)
            x = self.linear(x)
            x = rearrange(x, 'n d h w c -> n d c h w').contiguous()
        elif self.mode == 'up':
            x = rearrange(x, 'n d (neiw neih c) h w -> n d (h neih) (w neiw) c', neih=2, neiw=2).contiguous()
            x = self.norm(x)
            x = self.linear(x)
            x = rearrange(x, 'n d h w c -> n d c h w').contiguous()
        else:
            print(f'please input right mode')
            raise Exception('wrong mode!')

        return x
    
class UpdownkeepSampling3D(nn.Module):
    def __init__(self, dim_in, dim_out, mode='keep'):
        super(UpdownkeepSampling3D, self).__init__()
        self.dim_in = dim_in
        self.dim_out = dim_out
        self.mode = mode
        if mode == 'down':
            self.norm = nn.LayerNorm(self.dim_in*2*2*2)
            self.linear = nn.Linear(self.dim_in*2*2*2, self.dim_out)
        elif mode == 'up':
            self.norm = nn.LayerNorm(self.dim_in//2//2//2)
            self.linear = nn.Linear(self.dim_in//2//2//2, self.dim_out)
        elif mode == 'keep':
    ...
    def forward(self, x):
        ...
        if self.mode == 'keep':
            ...
            x = rearrange(x, 'n (d neid) c (h neih) (w neiw) -> n d h w (neid neih neiw c)', neid=2, neih=2, neiw=2).contiguous()
            x = self.norm(x)
            x = self.linear(x)
            x = rearrange(x, 'n d h w c -> n d c h w').contiguous()
        elif self.mode == 'up':
            x = rearrange(x, 'n d (neid neih neiw c) h w -> n (d neid) (h neih) (w neiw) c', neid=2, neih=2, neiw=2).contiguous()
            x = self.norm(x)
            x = self.linear(x)
            x = rearrange(x, 'n d h w c -> n d c h w').contiguous()
        else:
